...
The experiment configurations are stored in the `configs/` folder. To load a configuration/experiment, specify the `config` argument in the url http://localhost:8000/?config=mushra_showresults.yaml. `configs/carnatic_listening_test.yaml` is the configuration loaded when no config is specified.
